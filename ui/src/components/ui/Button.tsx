...
const VARIANTS: Record<ButtonVariant, string> = {
  primary:
    "bg-primary-500 text-white border border-primary-500 hover:bg-primary-600 hover:border-primary-600",
  secondary:
    "bg-bg-surface text-text-base border border-border-base hover:bg-bg-elevated hover:border-border-strong",
  danger:
    "bg-transparent text-signal-danger border border-signal-danger/30 hover:bg-signal-danger/10 hover:border-signal-danger/50",
  ghost:
    "bg-transparent text-text-muted border border-transparent hover:bg-bg-elevated hover:text-text-base",
};
...
export const Button = ({
  ...
  return (
    <button
      ...
      className={getButtonClasses({ variant, size, isLoading, className })}
      {...props}
    >
      {isLoading ? (
        <>
